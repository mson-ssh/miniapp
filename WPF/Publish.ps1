...
if ($Target -eq 'both') {
    ...
    # Preserve the published v0.3.6 fallback byte-for-byte; do not rebuild net10.
    $fallbackFile = "MiniApps-net10-$Runtime.zip"
    $fallbackZip = Join-Path $artifacts $fallbackFile
    $fallbackHash = 'e1cbb8c392061b92d19fa0a9118e8351a298d4b11a9d4233bd4058723801f515'
    $fallbackSize = 63080335
    ...
    if (-not $reuseVerified) {
        Invoke-WebRequest "https://github.com/mson-ssh/miniapp/releases/download/v0.3.6/$fallbackFile" -OutFile $fallbackZip -UseBasicParsing -TimeoutSec 600
    }
    ...
    Write-Host 'Reused net10 v0.3.6 unchanged; no net10 build was performed.'
}
