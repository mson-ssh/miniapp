...
Script MiniApps: `MiniApps/Scripts/Optimize-Defaults.ps1`, chỉ đóng gói cho net48. Fork hẹp từ Win11Debloat đã ghim được lưu trong `ThirdParty/Win11Debloat` và đóng gói ra `Engine/Debloat`; Optimize không tải ZIP/source từ GitHub khi chạy. Mỗi lượt sao chép engine sang thư mục phiên ngắn, riêng rồi bỏ restore point khỏi profile. Engine có chế độ host `-MiniApps`, nạp adapter tiến trình trực tiếp và nhận đường backup/deadline rõ ràng; không còn chèn bridge bằng regex. Xác minh kích thước/SHA-256 của gói phát hành MiniApps trong bootstrap vẫn được giữ nguyên. Xem [DEBLOAT-REWRITE.md](DEBLOAT-REWRITE.md).
