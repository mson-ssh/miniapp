# Tài liệu MiniApps WPF

Đây là điểm bắt đầu cho người phát triển và AI trên máy mới. Các đường dẫn trong tài liệu tính từ `WPF/`, trừ khi nói khác.

| Đọc | Nội dung |
| --- | --- |
| [AGENTS.md](../AGENTS.md) | Hướng dẫn AI và phạm vi thao tác |
| [PROJECT-RULES.md](PROJECT-RULES.md) | Quyết định sản phẩm, runtime và phát hành |
| [CURRENT-STATE.md](CURRENT-STATE.md) | Trạng thái bàn giao, giới hạn và việc còn lại |
| [ARCHITECTURE.md](ARCHITECTURE.md) | Sơ đồ source, cấu hình và luồng thực thi |
| [DEVELOPMENT.md](DEVELOPMENT.md) | Chuyển máy, build/test/preview và GitHub |

Các kế hoạch `IMPLEMENTATION-PLAN.md`, `NET48-MIGRATION-PLAN.md`, `DUAL-RUNTIME-FALLBACK.md` ở thư mục WPF là tài liệu lịch sử; đối chiếu code và trạng thái hiện tại trước khi áp dụng. README/ARCHITECTURE/workflow ở ngoài WPF chủ yếu mô tả CLI cũ.

Khi bàn giao, cập nhật `CURRENT-STATE.md`: ngày, phạm vi thay đổi, target được kiểm thử, kết quả, release thực tế, việc chưa xác minh. Không ghi token, mật khẩu hoặc thông tin máy khách vào docs.
