...
- MiniApps luôn yêu cầu quyền Administrator ngay khi khởi động qua manifest `requireAdministrator`, áp dụng cho Developer và Public.
...
- Sau thay đổi tính năng, kiểm tra phù hợp rồi build/mở Developer net48 thật bằng `Run-Developer.ps1`. Không tự bấm chạy cài đặt, Optimize hay script cấu hình Windows; người dùng trực tiếp quyết định thao tác thật trong giao diện Developer.
- Developer dùng cấu hình thật trong `ReleaseConfig`; không tự sửa/lưu cấu hình chỉ để thử UI. `Preview.ps1` vẫn dành cho fixture hoặc khi người dùng yêu cầu mô phỏng.
