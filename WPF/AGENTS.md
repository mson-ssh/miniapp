...
- MiniApps luôn yêu cầu quyền Administrator ngay khi khởi động qua manifest `requireAdministrator`.
...
- Từ 2026-09-18 chỉ còn một bản **Public**; edition Developer (trang Setting, `Run-Developer.ps1`, cờ `MiniAppsEdition`) đã bị gỡ theo yêu cầu người dùng.
- Sau thay đổi tính năng, kiểm tra phù hợp rồi build/mở net48 trong output riêng, kèm bản sao `ReleaseConfig`. Không tự bấm chạy cài đặt hay script cấu hình Windows; người dùng trực tiếp quyết định thao tác thật trong giao diện Public.
- Catalog nằm trong `ReleaseConfig/apps.json` và `windows.json`, sửa trực tiếp file JSON; không tự sửa cấu hình chỉ để thử UI. `Preview.ps1` dành cho khi người dùng yêu cầu mô phỏng.
